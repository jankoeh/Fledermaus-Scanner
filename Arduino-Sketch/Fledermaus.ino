...
int nDataMode = DM_RAW;      // 0 = raw, 1 = FFT, 2 = Frequenzen
...
void adc_init()
{
  ...
  ADC->SAMPCTRL.reg         = 0x01;  ; //sample length in 1/2 CLK_ADC cycles - JK: changed from 0 to 1 to fix oscillation issue with high and low values
  ADCsync();
...
void send_freq_data(int nThres)
{
    for( int i=0; i<HWORDS; i++ ) signal[i] = adcbuf[i]-512;     // DC Wert abziehen
    ZeroFFT( signal, HWORDS );          // FFT rechnen
    int nFoundEcho = 0; 
...
void loop()
{
    int nTimeout = TIMEOUT_MS;
    if( Serial.available() )
    {
      int nRead = Serial.read();
      if( (DM_RAW <= nRead) && (nRead <= DM_FREQ) )
        nDataMode = nRead;
    }
    digitalWrite(LED_BUILTIN, HIGH);
    adc_dma(adcbuf,HWORDS);           // Start des Samplings
    while( (!dmadone) && (nTimeout>0) )
    {   // Warten, bis Sampling und DMA fertig (nTimeout notwendig)
      delay( 1 );
      nTimeout--;                  // Warten, bis Sampling und DMA fertig
    }
